...
type DesignTab = 'templates' | 'editor' | 'generator' | 'catalog'
          ...
          <div className={`mt-4 p-3 rounded-lg text-[11.5px] ${allOk ? 'bg-emerald-50/60 text-emerald-800' : 'bg-rose-50 text-rose-800'}`}>
            ...
            <ul className="space-y-0.5 ml-5 list-disc">
              {constraintResults.map((c, i) => (
                <li key={i} className={c.ok ? '' : 'font-medium'}>{c.rule}</li>
              ))}
            </ul>
          </div>
        )}
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <div className="text-[16px] font-semibold tracking-tight text-stone-900 mt-0.5">{resolvedParts[selectedPart].name}</div>
                </div>
                <button className="text-[11px] text-rose-600 hover:underline">Törlés</button>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: 'Anyag', value: matLabel(resolvedParts[selectedPart].rMat), raw: tpl.parts[selectedPart].mat },
                  { label: 'Szélesség', value: `${resolvedParts[selectedPart].rW} mm`, raw: tpl.parts[selectedPart].w },
                  { label: 'Magasság', value: `${resolvedParts[selectedPart].rH} mm`, raw: tpl.parts[selectedPart].h },
                  { label: 'Vastagság', value: `${resolvedParts[selectedPart].rT} mm`, raw: tpl.parts[selectedPart].t },
                  { label: 'Mennyiség', value: `${resolvedParts[selectedPart].rQty} db`, raw: tpl.parts[selectedPart].qty },
                ].map((f) => (
...
function MaterialsGenerator() {
  ...
  const [orderRef] = useState('JT-2426-0184 — Bognár Bútor Kft.')
        ...
        <div className="space-y-4">
          <Card className="p-0 overflow-hidden">
            <div className="px-5 py-3 border-b border-stone-100">
              ...
              <div className="text-[11px] text-stone-500">{resolved.length} alkatrész · {orderRef}</div>
            </div>
            <table className="w-full text-[12px]">
              <thead>
              ...
              <tbody>
                {resolved.map((p, i) => (
                  <tr key={i} className="border-b border-stone-50 last:border-0">
                    <td className="px-5 py-2.5 font-medium text-stone-900">{p.name}</td>
                    <td className="px-5 py-2.5 text-stone-700">
                      <span className="inline-flex items-center gap-2">
  ...
  return (
    <div>
      <div className="flex items-center gap-2 flex-wrap mb-4">
        <span className="text-[10.5px] uppercase tracking-wide text-stone-500 font-medium">Anyag katalógus</span>
  ...
  return (
    <div className="px-7 py-5 max-w-[1600px] mx-auto">
      <div className="flex items-center gap-2 mb-4">
        <span className="text-[13px] font-semibold text-stone-900 mr-1">Tervezés</span>
      ...
      {tab === 'templates' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {TEMPLATES.map((tmpl) => (
              <Card key={tmpl.id} className="p-5" interactive>
                ...
                <button
                  ...
                  className="mt-3 w-full text-center text-[12px] font-medium text-amber-700 hover:text-amber-900 py-1.5 border border-amber-200 rounded-lg hover:bg-amber-50 transition"
                >
                  Megnyitás
                </button>
              </Card>
      ...
      {tab === 'editor'    && <TemplateEditor />}
      {tab === 'generator' && <MaterialsGenerator />}
      {tab === 'catalog'   && <DesignCatalog />}
    </div>
  )
}
